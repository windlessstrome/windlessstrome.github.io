...
if(!System.bundlesPath) {
	System.paths["bundles/*.css"] ="../../../../../../endo/build/production/endo/notification/*css";
	System.paths["bundles/*"] = "../../../../../../endo/build/production/endo/notification/*.js";
}
System.bundles = {"bundles/endo/notification.css!":["endo/components/windows-notification/windows-notification.less!$less","common/components/button/button.less!$less"]};
/*stealconfig.js*/
define('stealconfig.js', function (require, exports, module) {
    steal.config({
        ...
        meta: {
            ...
            jqueryarraybuffer: { deps: ['jquery'] },
            jquerytextfill: { deps: ['jquery'] },
            fileupload: { deps: ['jqueryuiwidget'] },
            statemachine: { exports: 'StateMachine' },
            caret: { deps: ['jquery'] },
            dropins: { exports: 'Dropbox' },
            recorder: { exports: 'Recorder' },
            zeroclipboard: { exports: 'ZeroClipboard' },
            'common/components/media_player/media_control/media_control.mustache': { deps: ['common/attributes/tooltip_hover/tooltip_hover'] },
            ...
            'can-binarytree': {},
            'can-derive': { deps: ['can-binarytree'] }
        },
        ext: {
...
define('can/view/stache/add_bundles', [], function(){ return {}; });
/*can/view/stache/system*/
define('can/view/stache/system', [], function(){ return {}; });
/*endo/notification.stache!can/view/stache/system*/
define('endo/notification.stache!can/view/stache/system', [
    'module',
    'can/view/stache/stache',
    'can/view/stache/mustache_core'
...
/*endo/components/windows-notification/windows-notification.stache!can/view/stache/system*/
define('endo/components/windows-notification/windows-notification.stache!can/view/stache/system', [
    'module',
    'can/view/stache/stache',
    'can/view/stache/mustache_core'
], function (module, stache, mustacheCore) {
    var renderer = stache([
        ...
        {
            'tokenType': 'special',
            'args': ['#unless notificationWithButtons']
        },
        {
            'tokenType': 'attrValue',
            'args': [' adjust-width ']
...
/*$less*/
define('$less', [], function(){ return {}; });
/*common/components/button/button.stache!can/view/stache/system*/
define('common/components/button/button.stache!can/view/stache/system', [
    'module',
    'can/view/stache/stache',
    'can/view/stache/mustache_core'
], function (module, stache, mustacheCore) {
...
define('common/components/button/button', [
    'can/component/component',
    './button.stache!',
    './button.less!'
    ...
    Component.extend({
        ...
        events: {
            inserted: function () {
                ...
                var attributes = $parent.prop('attributes');
                $.each(attributes, function () {
                    if ($.inArray(this.name, ignore) === -1) {
                        var origAttr = $button.attr(this.name);
                        if (this.name) {
                            $button.attr(this.name, this.value + (origAttr ? ' ' + origAttr : ''));
                            $parent.attr(this.name, null);
                        }
                    }
                });
                if ($parent.hasOwnProperty('disabled')) {
                    $button.prop('disabled', true);
                }
                if ($parent.attr('disabled')) {
                    $button.prop('disabled', true);
                }
                if (this.scope.attr('focus')) {
                    $button.focus();
                }
            },
            '{scope} disabled': function (scope, ev, disabled) {
...
/*endo/components/windows-notification/windows-notification*/
define('endo/components/windows-notification/windows-notification', [
    'can/util/string/deparam/deparam',
    'can/view/view',
    'can/component/component',
    './windows-notification.stache!',
    './windows-notification.less!',
    'common/components/button/button',
    'can/map/define/define'
], function (can, view, Component, template) {
    var win = nw.Window.get();
    Component.extend({
        tag: 'st-windows-notification',
        template: template,
        scope: {
            name: '@',
            status: '@',
            ...
            type: '@',
            callId: '@',
            notificationSource: 'Mitel Connect',
            define: {
                notificationWithButtons: {
                    type: 'boolean',
                    get: function () {
                        return this.attr('type') === 'call' || this.attr('type') === 'sh';
                    }
                },
                isCall: {
                    type: 'boolean',
                    get: function () {
                        return this.attr('type') === 'call';
                    }
                },
                conference: {
                    get: function () {
                ...
                hasMultipleShareRequests: {
                    type: 'boolean',
                    get: function () {
                        return this.attr('requestedPresenters.length') > 1;
                    }
                },
                hasMailBox: { type: 'boolean' },
                answerCall: { type: 'boolean' }
            },
            acceptCall: function () {
                if (win.parent_self) {
                    win.parent_self.callAction('accept', win.callId || win.imId);
                    this.closeNotification(true);
                }
            },
            rejectCall: function () {
                if (win.parent_self) {
                    win.parent_self.callAction('reject', win.callId || win.imId);
                    this.closeNotification(true);
                }
            },
            getAttendee: function (attendeeId) {
                var conference = this.attr('conference'), attendee = null;
                can.each(conference.requestedPresenters, function (requestedPresenter) {
                    if (requestedPresenter.from === attendeeId) {
                        attendee = requestedPresenter;
                    }
                });
                return attendee;
            },
            acceptShare: function (attendeeId) {
                var attendee = this.getAttendee(attendeeId);
                this.attr('conference').acceptShareRequest(attendee);
                this.closeNotification(true);
            },
            rejectShare: function (attendeeId) {
                var attendee = this.getAttendee(attendeeId);
                this.attr('conference').rejectShareRequest(attendee);
                this.closeNotification(true);
            },
            closeNotification: function (clicked) {
                clicked && win.parent_self.handleClickFromNotifcation(win.callId || win.imId, this.type, win.parent_self);
                if (this.attr('type') === 'sh') {
                    win.parent_self.handleShareNotificationClose(win.confCode, win.callId);
                }
                win.close();
            }
        },
        events: {
            ' inserted': function () {
                var self = this;
                setTimeout(function () {
                    self.scope.closeNotification();
                }, this.scope.attr('timeout'));
            },
            ' click': function () {
                if (win.parent_self && !this.scope.attr('isCall')) {
                    this.scope.closeNotification(true);
                }
            }
        },
        helpers: {
            avatarUrl: function () {
                if (win.parent_self && win.parent_self.contact) {
                    this.attr('contact', win.parent_self.contact);
                    if (this.attr('contact.adPicture')) {
                        return this.attr('contact.adPictureData');
                    } else {
                        return 'images/icons/Avatar_small.png';
                    }
                }
                if (this.attr('type') === 'sh') {
                    return 'images/shoretel-icon.png';
                }
                return 'images/icons/Avatar_small.svg';
            }
        }
    });
});
/*endo/notification*/
define('endo/notification', [
    './notification.stache!',
    'can/view/stache/stache',
    'endo/components/windows-notification/windows-notification'
], function (template) {
